...
- "score": A number between 0 and 100 representing the matching compatibility score percentage. If the resume is for a completely different profession than the job description (e.g. software engineer vs doctor), this score MUST be 0 or close to 0.
- "formattingScore": A number between 10 and 100 representing the ATS formatting readability and structural integrity score based on standard guidelines. This score measures structure (email, phone, headers) and is independent of profession mismatch.
...
- "formattingIssues": An array of objects representing specific ATS formatting or structural issues found in the resume. Each object must have:
  - "issue": A brief description of the issue found (e.g., "Missing LinkedIn Profile", "Unusual section heading 'What I Do' instead of 'Work Experience'").
  - "severity": A string: either "error" (for critical issues like missing sections/contact info) or "warning" (for minor formatting/structural suggestions).
  - "fix": A clear, actionable suggestion on how the candidate can fix this formatting/structural issue.
